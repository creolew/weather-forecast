...
@WebMvcTest(LocationApiController.class) //load only Spring MVC components, ex: REST Controllers
public class LocationApiControllerTest {

	private static final String END_POINT_PATH = "/v1/locations";
	
	@Autowired 
	MockMvc mockMvc; //perform api calls and assertion the responses 
	
	@Autowired
	ObjectMapper mapper; //serializes Java objs to JSON and vice versa
	...
	public void testValidateRequestBodyAllFieldsInvalid() throws Exception {
		...
		MvcResult mvcResult = mockMvc.perform(put(END_POINT_PATH).contentType("application/json").content(bodyContent))
		.andExpect(status().isBadRequest())
		.andExpect(content().contentType("application/json"))
		.andDo(print())
		.andReturn();
		...
		assertThat(responseBody).contains("Region name code must have 3-128 characters");
